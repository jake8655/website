...
void main() {
    ...
    // Define blue and green as the two colors
    vec3 blue = vec3(0.0, 0.0, 1.0);  // Pure blue
    vec3 green = vec3(0.0, 1.0, 0.3); // Pure green

    // Create a gradient value based on vUv.y
    float gradient = vUv.y; // Gradient changes along the y-axis

    // Interpolate between blue and green
    vec3 color = mix(blue, green, gradient);
  ...
  return (
    <div className="-z-10 absolute top-0 h-full w-full">
      <div className="sticky top-0 h-[100dvh] opacity-70">
        ...
        <div className="-z-20 -translate-x-1/2 -translate-y-1/2 !transform-gpu absolute top-1/2 left-1/2 h-[600px] max-h-[100dvh] w-[600px] max-w-[100vw] rounded-full bg-linear-to-b from-green-500 to-blue-800 opacity-70 blur-[150px]"></div>
      </div>
    </div>
  );
}
